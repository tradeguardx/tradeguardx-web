...
const stats = [
  {
    value: 'Rule Engine',
    label: 'Real-time trade enforcement',
    icon: (
...
export default function StatsStrip() {
  return (
    <section className="relative border-y border-white/[0.05] py-10">
      <div className="pointer-events-none absolute inset-0 bg-gradient-to-r from-transparent via-accent/[0.025] to-transparent" />
      <div className="relative mx-auto max-w-5xl px-6">
        <motion.div
          initial={{ opacity: 0, y: 12 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          className="grid grid-cols-2 gap-6 md:grid-cols-4 md:gap-0 md:divide-x md:divide-white/[0.06]"
        >
          {stats.map((s, i) => (
            <motion.div
              key={s.label}
            ...
            >
              <span className={`${s.color} opacity-75`}>{s.icon}</span>
